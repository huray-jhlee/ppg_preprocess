...
# SQA_DECISION_THRESHOLD = -0.05
SQA_DECISION_THRESHOLD = 0
...
def sqa(
    ...
    for idx, segment in enumerate(segments):
        ...
        if np.isnan(np.array(features)).any():
            ...
            score = model.decision_function(features_norm)[0]
            pred = 0 if score >= SQA_DECISION_THRESHOLD else 1
...
if __name__ == "__main__":
    ...
    # input_sig = get_data(file_name=file_name, whitespace=True)
    # input_sampling_rate = 20
    
    file_name = "ppg_chunk_10_20250904_011944_to_20250904_012744.csv"
    input_sig = get_data(file_name=file_name, whitespace=False)
    input_sampling_rate = 25
